...
| internal.producer.retries | \>= 0 | 5 | Sets the number of times to retry sending a message from the internal queue before giving up. This value should not be set concomitantly with RdKafka option **message.send.max.retries**. As such the latter will be set to **0** if **internal.producer.retries>0**. |
